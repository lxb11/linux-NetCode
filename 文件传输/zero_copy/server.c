                    ...
                    else
                    {
                        ...
                        for(int k = 2; k < _MAX_FD_NUM_; k++)
                        {
                            if(nfd[k].fd > 0 && nfd[k].revents & POLLOUT)
                            {
                                ...
                                fstat(file_fd, &stat_info);
                                FILE * fd = fdopen(file_fd, "rb");//取一个现存的描述符
                                ...
                                if(sendfile(nfd[k].fd, file_fd, NULL, stat_info.st_size) < 0)
                                {
                                    perror("sendfile");
                                    exit(1);
                                }
                                char *flag = "end";//结束标志
                                if(writen(nfd[k].fd, flag, sizeof(flag)) < 0)//发送文件结束标志
                                {
                                    perror("writen");
                                    continue;
                                }
